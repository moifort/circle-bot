...
describe('Evaluator', () => {
  it('should take the bet if odds are good', () => {
    // When
    const decision = decideFavorite(Percentage(0.9), PolymarketPrice(0.6), Amount(1000))

    // Then
    expect(decision.isOk()).toBe(true)
    expect(decision.getOrThrow()).toEqual({ amountToBet: Amount(750) })
  })
  ...
  it('should not take the bet if unprofitable bet', () => {
    // When
    const decision = decideFavorite(Percentage(0.9), PolymarketPrice(0.01), Amount(10))
